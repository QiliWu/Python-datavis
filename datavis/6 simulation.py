...
mu = 1
NumSimulation= 6
colors = [(214, 27, 31), (148, 102,189), (229, 109, 0), (41, 127, 214), 
...
for j in range(0, NumSimulation, 1):
    ...
    plt.plot(x, S[j], linewidth=2.0, color=colors[j], label = str(j))
    print colors[j]
    
plt.title('%d Simulation using %d Steps \n$\sigma$=%.6f $\mu$=%.6f$S_0$=%.6f ' % (int(NumSimulation), 
int(Steps), sigma, mu, S_init), fontsize=18)
